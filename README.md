...
```sh
go get -u 'github.com/VAT-Sense/vatsense-go@v0.0.1'
```
...
```go
params := FooParams{
	Name: vatsense.F("hello"),

	// Explicitly send `"description": null`
	Description: vatsense.Null[string](),

	Point: vatsense.F(vatsense.Point{
		X: vatsense.Int(0),
		Y: vatsense.Int(1),

		// In cases where the API specifies a given type,
		// but you want to send something else, use `Raw`:
		Z: vatsense.Raw[int64](0.01), // sends a float
	}),
}
```
...
See the [full list of request options](https://pkg.go.dev/github.com/VAT-Sense/vatsense-go/option).
...
We also provide a helper `vatsense.FileParam(reader io.Reader, filename string, contentType string)`
...
```go
params := FooNewParams{
    ID:   vatsense.F("id_xxxx"),
    Data: vatsense.F(FooNewParamsData{
        FirstName: vatsense.F("John"),
    }),
}
...
We are keen for your feedback; please open an [issue](https://www.github.com/VAT-Sense/vatsense-go/issues) with questions, bugs, or suggestions.
